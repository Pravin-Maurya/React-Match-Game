...
class App extends Component {
  state = {
    ...
    time: 60,
    isTimeOut: false,
    largeImgUrl: imagesList[0].imageUrl,
  }
  ...
  clickImage = id => {
    const {largeImgUrl} = this.state
    const largeImageList = imagesList.filter(eachValue => eachValue.id === id)
    const {imageUrl} = largeImageList[0]
    if (largeImgUrl === imageUrl) {
      const newImgUrl =
  ...
  render() {
    const {score, time, largeImgUrl, isTimeOut, activeTabId} = this.state
    const filteredImagesList = this.getFilteredImageList()
    return (
        ...
        <div className="match-container">
          {!isTimeOut && (
            <div className="img-tab-container">
              <img src={largeImgUrl} alt="match" className="large-img" />
              <ul className="tab-list-items">
              ...
              <ul className="image-list-items">
                {filteredImagesList.map(ItemDetails => (
                  <MatchItem
